...
void display_Registers_settings(void){
   ...
   for(uint8_t address =0; address<24; address++){  
       Serial.print("Register #: 0x");  Serial.print(address,HEX); 
       Serial.print(" \t");
       Serial.print(Registers_NTable[address]);
       Serial.print("\t");
       if(!(address>12 && address <20))Serial.print("\t");
       Serial.print("Config Bits: 0x"); Serial.print(readRegisterSettings(address),HEX);
       Serial.print(" \t");
       Serial.print("Bin: 0b");         Serial.print(readRegisterSettings(address),BIN);
       Serial.println();
   }
   Serial.println("-------------------------------------------------------------------------------");
}
 ...
 * @param[in]  data     The configuration value data byte
 * 
 * @return returnedByte byte holds the target register address
 */
...
byte stop_and_configInput(uint8_t board_id, uint8_t channel, byte data){ 
      ...
      byte REGadress = config_ADS_Input(channel,data);
  return REGadress;
}
 ...
 * @brief      Single Shot Mode - Get Data in shared Union Structure: "ADC_data".
 */
void get_ADC_Data_SSAlt(adc_data * adc){  // Single Shot mode get data via interrupt   
     digitalWrite(ADS_CS_pin, LOW); 
     NovaXR_SPI.beginTransaction(SPISettings(SPI_CLK, MSBFIRST, SPI_MODE1));        
     NovaXR_SPI.transfer(START);
     NovaXR_SPI.transfer(RDATA);  // set single shot data read
...
void ADS_Init_Config(uint8_t ads_id){
   ...
   if(!ADS_connected()){ // Check if device is attached to the board correctly 
        ...
        config_ADS_Register(CONFIG4,0x00);   /// CONFIG4 Register: 
 ...
 *
 * @param[in]  ads_id    The ADS1299 board identifier
...
/**
 * @brief      Convert Raw Data to micro Volts
 *
 * @param[in]  raw_data  The raw data from the ADS1299
 * @param[in]  gain      The defined gain of the ADS1299
 *
 * @return     { description_of_the_return_value }
 */
float convert_ADC_volts(uint32_t raw_data, uint8_t gain,float vref ){
    float converted_data = (vref / gain) * (raw_data / (2^(23)-1));
    return converted_data;
}


/**
 * @brief      Concatinate 3 Bytes in a single Value
 *
 * @param[in]  Byte1  The byte 1
 * @param[in]  Byte2  The byte 2
 * @param[in]  Byte3  The byte 3
 *
 * @return     { description_of_the_return_value }
 */
...
bool print_Single_Shot(void){         
      bool error =false;
      long a = micros();
      get_ADC_Data_SS(&MBdataADC); // single readback 
      //get_ADC_Data_SSAlt(&SBdataADC);           
      long b = micros();
      for(uint8_t r=0; r<27; r++){
        Serial.print(SBdataADC.rawData[r],HEX);Serial.print(",");
        if(MBdataADC.rawData[0]==0xFF){
          error = true;
          break;
         }    
       }
...
/**
 * @brief      Diplay All ADS1299 Devices Configurations
 */
