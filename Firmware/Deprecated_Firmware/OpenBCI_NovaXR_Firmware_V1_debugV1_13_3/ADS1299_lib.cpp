...
// ------------- Print Current ADS1299 that is in use -----------------
...
// -------- For ADC1 on mainbaord - Interrupt Routine to Get Data in shared Union Structure: "ADC_data"  ----------------------
...
bool print_Single_ShotAlt(void){         
      bool error =false;
      long a = micros();
      get_ADC_Data_SSAlt(); // single readback            
      long b = micros();
      for(uint8_t r=0; r<27; r++){
        Serial.print(rawADC.rawData[r],HEX);Serial.print(",");
        if(rawADC.rawData[0]==0xFF){
          error = true;
          break;
         }    
       }
