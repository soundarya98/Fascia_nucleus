...
#include "NovaXR_Comm_Protocol.h"
 ...
 if (checkMultiCharCmdTimer()){ // we are in a multi char command  
   ...
   switch (character){
    ...
    case OPENBCI_CHANNEL_CMD_SET: // This is a multi char command with a timeout
      Serial.print("OPENBCI_CHANNEL_CMD_SET: ");Serial.println(OPENBCI_CHANNEL_CMD_SET);
...
void processMultiChar(void){
   ...
   if(ch_Input<9){ // if main board input is requested to be changed
    ...
    byte targetReg = stop_and_configInput(Board_ID_num, config_channel, CHANNEL_ON | Gain_Option | SRB2_Option |Input_Option );
    
    byte config_value1 = readRegisterSettings(targetReg);
   ...
   //config_BIAS_LOFF(POSITIVE, BIAS, ENABLE, 3);
    ...
    byte config_value2 = readRegisterSettings(CONFIG3);

    config_ADS_Register(MISC1, SRB1_Option);
    byte config_value3 = readRegisterSettings(MISC1);
