...
// --- for any ADS1299 -----------
uint8_t ADS_Reset_pin;             // Pin Variable initialization 
...
void ADS_Init_Config(uint8_t ads_id){
   ...
   if(!ADS_connected()){ // Check if device is attached to the board correctly 
        ...
        config_All_ADS_Inputs(CHANNEL_OFF | ADS_GAIN04 | CH_SRB2_DISCONNECTED | CH_NORMAL_ELECTRODE);   // Active channels
        delay(10);
...
/**
 * @brief     ADS1299 exaple that prints RAW values in  single conversion mode
 */
