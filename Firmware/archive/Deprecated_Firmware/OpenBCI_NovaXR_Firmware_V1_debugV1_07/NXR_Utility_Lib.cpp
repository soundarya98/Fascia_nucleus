...
void Connect_to_AP(const char* ssid, const char* pass ){
...
bool ask_for_WiFi_Cred(void){

    // ...in this case we ask for user data.
    Serial.setTimeout(30000);
    Serial.println("Insert your wifi SSID:");
    String ssid_str = Serial.readStringUntil('\n');
    Serial.println(ssid_str);
    Serial.println("Insert your wifi Password:");
    String pass_str= Serial.readStringUntil('\n');
    Serial.println(pass_str);
    
    // Fill the "WiFi crentials" structure with the data entered by the user...
      ssid_str.toCharArray(wfSSID, 20);
      pass_str.toCharArray(wfPASS, 20);
