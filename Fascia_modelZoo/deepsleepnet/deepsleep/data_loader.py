...
class NonSeqDataLoader(object):
    ...
    def _load_npz_list_files(self, npz_files):
        ...
        for npz_f in npz_files:
            ...
            # print("first shape", tmp_data.shape)
            if fs is None:
                fs = sampling_rate
    ...
    def load_train_data_psg(self, n_files=None):
        psg_dirs = ['data/eeg_fpz_cz', 'data/eeg_pz_oz', 'data/emg', 'data/eog', 'data/resp-oro-nasal', 'data/temp']
        x_train_psg = []
        y_train_psg = []
        data_val_psg = []
        label_val_psg = []
        ...
        for dir in psg_dirs:
            # Remove non-mat files, and perform ascending sort
                ...
                for idx, f in enumerate(allfiles):
                    if ".npz" in f:
                        npzfiles.append(os.path.join(dir, f))
                npzfiles.sort()
                ...
                for idx, f in enumerate(allfiles):
                    if self.fold_idx < 10:
                        ...
                        pattern = re.compile("[a-zA-Z0-9]*{}[1-9]E0\.npz$".format(self.fold_idx))
                    if pattern.match(f):
                        subject_files.append(os.path.join(dir, f))
                ...
                print("Load training set:")
                data_train, label_train = self._load_npz_list_files(npz_files=train_files)
                print("ORIGINAL DATA TRAIN SHAPE", data_train.shape)
                print(" ")
                print("Load validation set:")
                data_val, label_val = self._load_npz_list_files(npz_files=subject_files)
                print(" ")
                ...
                if(flag==False):
                    x_train_psg = x_train
                    y_train_psg = y_train.reshape(-1, 1)
                    data_val_psg = data_val
                    label_val_psg = label_val.reshape(-1, 1)
                    flag = True
                else:
                    print("SHAPES", x_train_psg.shape, y_train_psg.shape, data_val_psg.shape, label_val_psg.shape)
                    # t3 = np.concatenate((t1.reshape(-1, 1), t2.reshape(-1, 1), axis=1)

                    x_train_psg = np.append(x_train_psg, x_train, axis = 2)
                    y_train_psg = np.append(y_train_psg, y_train.reshape(-1, 1), axis=1)
                    data_val_psg = np.append(data_val_psg, data_val, axis=2)
                    label_val_psg = np.append(label_val_psg, label_val.reshape(-1, 1), axis=1)

        x_train_psg = np.array(x_train_psg)
        y_train_psg = np.array(y_train_psg)
        data_val_psg = np.array(data_val_psg)
        label_val_psg = np.array(label_val_psg)

        return x_train_psg, y_train_psg, data_val_psg, label_val_psg
