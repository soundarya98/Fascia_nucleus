...
from testNPZ import load_single_epoch
from testNPZ import iterate_batch_seq_minibatches_single_epoch
...
def custom_run_single_instance_epoch(
    ...
    all_bw_memory_cells = []
    total_loss, n_batches = 0.0, 0
    each_x = np.array(inputs)
    each_y = np.array(targets)
    ...
    each_x = each_x[:, :, np.newaxis, np.newaxis]

    each_x_orig = each_x
    each_y_orig = each_y

    for i in range(24):
        each_x = np.vstack([each_x, each_x_orig])
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        ...
        for subject_idx in range(n_subjects):
            ...
            for channel in ['eeg_fpz_cz', 'eeg_pz_oz', 'emg', 'eog', 'resp_oro_nasal']:
                ...
                npz_f = os.path.join(data_dir, '30sec', channel, filename)
                # Load testing data
                x, y = load_single_epoch(channel, npz_f)
